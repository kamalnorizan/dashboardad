                ...
                <div class="ibox float-e-margins">
                            ...
                            <div class="col-md-8">
                             ...
                             <div class="col-lg-2 text-center">
                                 <div class="form-group{{ $errors->has('tahun') ? ' has-error' : '' }}">
                                     {!! Form::select('tahun',$tahun, date('Y')-1, ['id' => 'tahun', 'class' => 'form-control', 'required' => 'required']) !!}
                                 </div>
                            </div>
                    ...
                    <div class="ibox-content">
                        ...
                        <div class="table-responsive center">
                            <table class="table table-striped center" id="tableLaporan">
                                <thead>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#tahun').change(function (e) {
            e.preventDefault();
            checklaporan($(this).val());
        });

        checklaporan((new Date).getFullYear()-1);

        function checklaporan(tahun){
            $.ajax({
                type: "get",
                url: "/ajaxbytahun/"+tahun,
                success: function (response) {
                    $('.rowlaporan').remove();
                        if(response.length==0){
                            $('#tableLaporan').append("<tr class='rowlaporan'><td colspan='5'><strong> Tiada data ditemui </strong></td></tr>")
                        }else{

                            $.each(response, function (indexInArray, data) {
                                var merah = data['merah'];
                                var kuning = data['kuning'];
                                var hijau = data['hijau'];
                                if(merah!=0){
                                    merah ="<a href='/detaillaporan/"+data['laporan_id']+"/7'>"+data['merah']+"</a>";
                                }
                                if(kuning!=0){
                                    kuning ="<a href='/detaillaporan/"+data['laporan_id']+"/9'>"+data['kuning']+"</a>";
                                }
                                if(hijau!=0){
                                    hijau ="<a href='/detaillaporan/"+data['laporan_id']+"/10'>"+data['hijau']+"</a>";
                                }
                                $('#tableLaporan').append("<tr class='rowlaporan'><td><strong>"+data['tajuk']+"</strong></td><td></td><td><h3>"+merah+"</h3></td><td><h3>"+kuning+"</h3></td><td><h3>"+hijau+"</h3></td></tr>")
